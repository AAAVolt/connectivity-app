...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [s, d, p, co, m, c] = await Promise.all([
  ...
  return (
    <div className="p-6 lg:p-8 w-full space-y-6">
      ...
      {/* Comarcas Tab */}
      {tab === "comarcas" && (
        <AreaAnalysisPanel areaType="comarca" areas={comarcas} regionScores={purposes} regionCoverage={coverage} />
      )}

      {/* Municipios Tab */}
      {tab === "municipios" && (
        <AreaAnalysisPanel areaType="municipality" areas={municipalities} regionScores={purposes} regionCoverage={coverage} />
      )}
    </div>
  );
}
